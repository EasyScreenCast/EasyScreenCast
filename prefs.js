...
const EasyScreenCastSettingsWidget = new GObject.Class({
    ...
    _initTabQuality(ctx, gtkDB, tmpS){
        ...
        tmpS.bind(Settings.ACTIVE_CUSTOM_GSP_SETTING_KEY,
            ...
            'active', Gio.SettingsBindFlags.DEFAULT);
        ctx.Ref_switch_CustomGSP.connect(
    ...
    _initTabWebcam(ctx, gtkDB, tmpS){
        ...
        ctx.Ref_StackSwitcher_WebCam = gtkDB.get_object(
            'sts_Webcam');
        //implements webcam stack obj
        ctx.Ref_StackObj_WebCam = gtkDB.get_object(
            'stk_Webcam');
        //implements webcam stack menu chooser
        ctx.Ref_Label_WebCam = gtkDB.get_object(
            'lbl_Webcam');
        //implements webcam caps stack menu chooser
        ctx.Ref_Label_WebCamCaps = gtkDB.get_object(
            'lbl_WebcamCaps');
    },
    ...
    _initTabFile(ctx, gtkDB, tmpS){
        ...
        ctx.Ref_stackswitcher_FileResolution.connect(
        ...
        ctx.Ref_filechooser_FileFolder.connect(
